...
import { FiPlus, FiMoreVertical, FiLock, FiX, FiSmile, FiSend, FiArrowLeft, FiLayout } from "react-icons/fi";
...
const Status = () => {
  ...
  useEffect(() => { if (user) fetchStatuses(); }, [user]);
  ...
  const handleUpload = async () => {
    ...
    } catch (err) { alert("Failed to upload status."); } finally { setUploading(false); }
  };
  ...
  const handleSendReply = async () => {
    ...
    try {
      await api.post("/messages", {
        ...
        content: replyText,
        messageType: "text",
        repliedStatus: { mediaUrl: currentStatus.mediaUrl, mediaType: currentStatus.mediaType, caption: currentStatus.content }
      });
      setReplyText("");
  ...
  const formatTime = (dateStr) => {
    ...
    if (diff < oneDay && date.getDate() === now.getDate()) return `Today at ${date.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
  ...
  return (
    <div className="chat-page">
      <div className="chat-container">
        <NavSidebar activePage="status" onMediaClick={() => setShowMediaModal(true)} />
          ...
          {selectedGroup ? (
            <div className="status-viewer">
              <div className="status-viewer-header">
                ...
                <div className="status-viewer-actions"><FiMoreVertical size={20} className="status-header-icon white-icon" /></div>
              </div>

              <div className="status-progress-container">{selectedGroup.statuses.map((_, i) => (<div key={i} className="status-progress-bg"><div className={`status-progress-fill ${i === selectedStatusIndex ? "active" : i < selectedStatusIndex ? "filled" : ""}`} /></div>))}</div>

              <div className="status-viewer-body">
                {currentStatus?.mediaUrl && (currentStatus.mediaType === "video" ? <video src={currentStatus.mediaUrl} autoPlay className="status-viewer-media" /> : <img src={currentStatus.mediaUrl} alt="Status" className="status-viewer-media" />)}
              ...
              {selectedGroup.user._id !== user._id && (
                <div className="status-viewer-footer">
                  <div className="status-reply-input-bar">
                    <FiSmile size={24} className="reply-icon" /><FiLayout size={24} className="reply-icon" />
                    <input type="text" placeholder="Type a reply..." className="status-reply-input" value={replyText} onChange={(e) => setReplyText(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendReply()} />
                    <div className="reply-send-btn" onClick={handleSendReply}><FiSend size={20} /></div>
                  </div>
                </div>
              )}
            </div>
      ...
      {isCreatorOpen && (
        <div className="status-creator-overlay">
          <div className="status-creator-header"><FiX size={24} className="status-header-icon" onClick={() => setIsCreatorOpen(false)} /><span className="status-creator-title">Add Status</span></div>
          <div className="status-creator-content">
            <div className="status-preview-container">
              ...
              <div className="status-input-wrapper">
                <input type="text" className="status-text-input" placeholder="Type a caption..." value={caption} onChange={(e) => setCaption(e.target.value)} />
